...
export const TaskDescription = ({ task }) => {
    ...
    return (<div className="rounded-lg border p-4">
      ...
      {isEditing ? (<div className="flex flex-col gap-y-4">
          <Textarea autoFocus placeholder="Add a description..." value={value} rows={4} onChange={(e) => setValue(e.target.value)} disabled={isPending}/>

          <Button size="sm" className="ml-auto w-fit" onClick={handleSave} disabled={isPending}>
            {isPending ? 'Saving...' : 'Save Changes'}
          </Button>
        </div>) : (<div>{task.description || <span className="italic text-muted-foreground">No description set...</span>}</div>)}
    </div>);
};
